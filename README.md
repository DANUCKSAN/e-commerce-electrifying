...
A Next.js App Router storefront for solar, storage, EV charging, and portable cooling products. The homepage streams the current product catalogue from Neon PostgreSQL through a Server Component and keeps filtering in URL-backed server rendering.
...
The storefront requires a configured `DATABASE_URL` and queries the active Neon catalogue on every request. It does not create or render fallback product records. Production requires `DATABASE_URL`, `BETTER_AUTH_SECRET`, and `BETTER_AUTH_URL` and fails fast if any are missing.
